...
const ChatPage = () => {
    ...
    const connectWebSocket = () => {
      ...
      client.connect({}, () => {
        ...
        client.subscribe(`/topic/room/${roomId}`, (message) => {
          ...
          // Validate timestamp and fallback to current time if invalid
          if (!newMessage.timeStamp || isNaN(new Date(newMessage.timeStamp))) {
  ...
  const sendMessage = async () => {
    if (stompClient && connected && input.trim()) {
      const message = {
        ...
        content: input,
        roomId: roomId,
        timeStamp: Date.now(), // Add timestamp when sending
      };
  ...
  return (
    <div className="relative">
      {/* Header */}
      <header className="dark:border-gray-700 fixed w-full dark:bg-gray-900 py-5 shadow flex justify-between items-center px-10 hidden sm:flex">
        <h1 className="text-xl sm:text-2xl font-semibold">Room: {roomId}</h1>
        <h1 className="text-xl sm:text-2xl font-semibold">User: {currentUser}</h1>
        <button
          onClick={handleLogout}
          className="dark:bg-red-500 dark:hover:bg-red-700 px-3 py-2 rounded-full text-sm sm:text-base"
        >
