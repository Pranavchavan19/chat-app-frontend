...
import { baseURL } from "../config/AxiosHelper";
import { getMessagess } from "../services/RoomService";
import { getSendTime } from "../config/helper"; // Assuming this function formats timestamp
  ...
  const sendMessage = async () => {
    if (stompClient && connected && input.trim()) {
      const message = {
        ...
        content: input,
        roomId: roomId,
        timeStamp: new Date().toISOString(), // current time when the message is sent
      };
            ...
            >
              <div className="flex flex-row gap-2">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <p className="text-sm sm:text-base">{message.content}</p>
                  <p className="text-xs sm:text-sm text-gray-400">
                    {getSendTime(message.timeStamp)} {/* Format the timestamp */}
                  </p>
                </div>
              </div>
            </div>
        ...
        <div className="w-full sm:w-3/4 lg:w-2/4 px-4 py-2">
          <div className="flex items-center justify-between rounded-full w-full">
            <input
              ...
              ref={inputRef}
              type="text"
              className="w-full px-4 py-2 border border-gray-300 dark:border-gray-700 rounded-full"
