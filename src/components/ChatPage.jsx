...
const ChatPage = () => {
  ...
  const chatBoxRef = useRef(null);
  const [stompClient, setStompClient] = useState(null);
  const [hasConnected, setHasConnected] = useState(false); // Track connection state to show toast only once

  useEffect(() => {
    // Load initial messages when roomId is available
    async function loadMessages() {
      try {
        const messages = await getMessagess(roomId);
        setMessages(messages);
  ...
  useEffect(() => {
    const connectWebSocket = () => {
      ...
      client.connect({}, () => {
        ...
        if (!hasConnected) {
          toast.success("Connected");
          setHasConnected(true); // Avoid showing "connected" toast again
        }
  ...
  const sendMessage = async () => {
    if (stompClient && connected && input.trim()) {
      const message = {
        ...
        content: input,
        roomId: roomId,
        timeStamp: Date.now(), // Timestamp added for timeAgo function
      };
  ...
  // Handle user logout and disconnect
  function handleLogout() {
    stompClient.disconnect();
    setConnected(false);
    setRoomId("");
